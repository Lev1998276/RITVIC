...
def creds(DB_NAME):
    ...
    if(DB_NAME == 'HCHB' or DB_NAME =='HCHB_DAS' or DB_NAME == 'VNSNY_BI' or DB_NAME == 'VNSNY_STAT'):
...
for i, row in enumerate(df_QE.itertuples()):
    if(str(row.TABLE_REFRESH_WHITE_LIST_ID)== TABLE_REFRESH_WHITE_LIST_ID):
        ...
        def db_name(DB_NAME):
            ...
            sql_db_list = ['HCHB','D365','HCHB_DAS','VNSNY_BI','VNSNY_STAT']
         ...
         #           client_session_keep_alive=True
            ...
            try:
                ...
                for count1 in new_list:
                    ...
                    SOURCE_DF = pd.read_sql_query("select * from " +row.SOURCE_DB +'.'+row.SOURCE_SCHEMA+'.'+row.SOURCE_TABLE_NAME+" order by " + source_headers.columns[0]+ " limit 10000 offset "+ str(count1) , sf_conn_2)
